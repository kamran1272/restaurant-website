...
export function StoreProvider({ children }) {
  ...
  const setRoomStatus = async (roomId, status) => {
    ...
    try {
      ...
      setServiceMode('online');
      showNotice('success', 'Room updated', `Room status changed to ${status}.`);
    } catch (error) {
      if (!error.status) {
        setServiceMode('offline');
        showNotice('warning', 'Room updated locally', 'The API is offline, so this change is currently local only.');
      }
    }
  };
